    ...
    return SingleChildScrollView(
      ...
      child: Column(
        ...
        children: [
          ...
          Text('Entre na sua conta'.toUpperCase(),
              textAlign: TextAlign.center, style: AppText.display(size: 24, letterSpacing: -0.5)),
          const SizedBox(height: 8),
          ConstrainedBox(
            constraints: const BoxConstraints(maxWidth: 270),
            child: Text(
              'Salve seus pedidos e pague mais rápido. O mesmo login também abre o painel do estabelecimento e da administração.',
              textAlign: TextAlign.center,
              style: AppText.body(size: 13, weight: FontWeight.w600, height: 1.5, color: AppColors.inkA(0.55)),
            ),
          ),
          const SizedBox(height: 22),
          AppButton.primary(label: 'Entrar ou criar conta', onPressed: () => context.go('/login')),
          const SizedBox(height: 10),
          AppButton.ghost(label: 'Rever apresentação do app', onPressed: () => context.go('/onboarding')),
        ],
      ),
    );
  }
